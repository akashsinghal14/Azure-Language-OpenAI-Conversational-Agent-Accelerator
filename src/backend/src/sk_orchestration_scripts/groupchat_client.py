...
async def main():
    async with DefaultAzureCredential(exclude_interactive_browser_credential=False) as creds:
        async with AzureAIAgent.create_client(credential=creds, endpoint=PROJECT_ENDPOINT) as client:
            ...
            head_support_agent_definition = await client.agents.get(AGENT_IDS["HEAD_SUPPORT_AGENT_ID"])
            ...
            translation_agent_definition = await client.agents.get(AGENT_IDS["TRANSLATION_AGENT_ID"])
